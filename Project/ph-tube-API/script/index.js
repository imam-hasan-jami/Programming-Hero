...
function loadVideoDetails(videoId) {
    ...
    fetch(url).then((response) => response.json()).then((data) => displayVideoDetails(data.video));
}
...
function displayCategories(categories) {
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        categoryDiv.innerHTML = `
            <button id="btn-${cat.category_id}" onclick="loadCategoryVideos(${cat.category_id})" class="btn btn-sm hover:bg-[#FF1F3D]  hover:text-white">
                ${cat.category}
            </button>
        `;
        ...
        videoCard.innerHTML = `
            <div class="card bg-base-100">
                <figure class="relative">
                    ...
                    <span
                    class="absolute bottom-2 right-2 text-sm rounded text-white bg-black px-2">3hrs 56 min ago</span>
                </figure>
            </div>
